    ...
    <div class="task__input mb-4">
      ...
      <div class=" bg-white rounded-lg overflow-hidden" v-else>
        ...
        <div class="input__footer px-4 py-2 text-end bg-zinc-50">
          <button type="button" role="button" class="btn bg-gray-400 me-3 hover:bg-gray-600"
            @click="closeAddTask">取消</button>
          <button type="button" role="button"
            class="btn bg-green-500 hover:bg-green-700 px-5 disabled:bg-green-300 disabled:bg-opacity-75"
            @click="buildTask" :disabled="task_title === ''">新增</button>
        </div>
      </div>
    </div>
    {{ currEditTask }}
          ...
          <div class="bg-white bg-opacity-70 mx-0.5 rounded-b h-0 overflow-hidden" :class="task.isOpen ? 'h-auto' : ''">
            ...
            <div class="px-4 py-2 mt-3 flex items-center justify-between border-t ">
              ...
              <button type="button"
                ...
                @click="currEditTask = JSON.parse(JSON.stringify(task))">編輯</button>
            </div>
          </div>
        </div>
        <div v-else>
          <div class=" bg-white rounded-lg overflow-hidden">
            <div class="px-2">
              <input type="text" v-model.trim="currEditTask.title" placeholder="What are you working on ?"
                class="py-2 mb-4 indent-1 w-full bg-transparent border-b-2 border-b-slate-500 outline-none text-xl md:text-2xl leading-none">
              <div class="flex items-center mb-3">
                <p>番茄鐘數量 :</p>
                <input type="number" class="ms-2 input-field w-16 bg-zinc-200" min="0" max="99" step="1"
                  v-model="currEditTask.totalPomodoro">
              </div>
              <div class="input__extra mb-5">
                <button type="button" role="button" v-if="!showEditNotes && currEditTask.notes?.trim() === ''"
                  @click="showEditNotes = true" class="btn bg-violet-500 me-3">+
                  註記</button>
                <textarea v-else v-model.trim="currEditTask.notes" rows="3" maxlength="250"
                  placeholder="take some notes"
                  class="bg-zinc-200 block mb-3 rounded w-full px-2 py-2 outline-none focus:ring focus:ring-gray-100"
                  @blur="isAreaEmpty"></textarea>
                <button type="button" role="button" v-if="!showEditPlans && !currEditTask.plans?.length"
                  @click="showEditPlans = true" class="btn bg-violet-500">+
                  計畫</button>
                <div v-else class="mt-4">
                  <h3 class="text-xl font-semibold" title="your plans">Plans</h3>
                  <ul class="mb-8">
                  ...
                  <button type="button"
                    class="w-fit mx-auto flex items-center justify-center text-gray-500 group hover:text-gray-700"
                    @click="addNewPlan(currEditTask.plans)">
                    <span class="material-symbols-outlined  me-1"
                      style="font-variation-settings:'FILL' 1,'wght' 500,'GRAD' 200">
                      add_circle
                    </span>
            ...
            <div class="input__footer px-4 py-2 text-end bg-zinc-50">
              ...
              <button type="button" role="button"
                class="btn bg-green-500 hover:bg-green-700 px-5 disabled:bg-green-300 disabled:bg-opacity-75"
                @click="editTask" :disabled="currEditTask.title === ''">儲存</button>
            </div>
          </div>
        </div>
      </li>
...
const isAreaEmpty = () => showEditNotes.value = currEditTask.value.notes ? true : false;
...
function closeAddTask() {
  ...
  task_title.value = '';
  notes.value = '';
  newPlans.value = [{ content: '', id: getUId(), pomodoro: 1, plans: [{ content: '', pomodoroCount: 1, id: getUId() }] }];
